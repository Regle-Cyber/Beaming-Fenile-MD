...
async function addExif(webpSticker, packname, author, categories = [''], extra = {}) {
    ...
    const stickerPackId = crypto.randomBytes(20).toString('hex');
    const json = {
        'sticker-pack-id': "by.kanako.v-" + stickerPackId,
    ...
    for (let func of [
        ...
        try {
            ...
            if (/image/i.test(type.mime)) {
                stiker = await sticker4(img, url)
                return await addExif(stiker.data, ...args)
            } else stiker = await func(img, url, ...args)

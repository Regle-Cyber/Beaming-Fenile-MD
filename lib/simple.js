...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        getFile: {
             ...
             */
            async value(PATH, fileName, saveToFile = true) {
                let res, filename
                ...
                if (data && saveToFile && !filename)(filename = path.join(__dirname, '../tmp/' + (fileName ?? new Date * 1) + '.' + type.ext), await fs.promises.writeFile(filename, data))
                return {
                    res,
                    filename,
                    ...type,
...
END:VCARD`.trim()
        ...
        sendStickerFromUrl: {
            ...
            async value(from, PATH, quoted, options = {}) {
                ...
                let types = await conn.getFile(PATH)
...
export function protoType() {
    ...
    Number.prototype.toTimeString = function toTimeString() {
        ...
        const days = Math.floor((this / (24 * 60 * 60 * 1000)) % 30)
        const months = Math.floor((this / (30 * 24 * 60 * 60 * 1000)) % 12)
        const years = Math.floor((this / (12 * 30 * 24 * 60 * 60 * 1000)))
        return (
            (years ? `${years} year(s) ` : '') +
